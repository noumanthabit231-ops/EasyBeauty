...
export default function Storefront({
  ...
  function goHome() { setStack([]); setShowAll(false); setQuery(''); }
  function jumpTo(index: number) { setStack((s) => s.slice(0, index + 1)); setShowAll(false); setQuery(''); }
  function back() { setQuery(''); if (showAll) { goHome(); return; } setStack((s) => s.slice(0, -1)); }
  ...
  // ---------- хелперы рендера (функции, а не вложенные компоненты — без ремаунта) ----------
  ...
  const breadcrumb = () => (
    <div className="mt-6 flex flex-wrap items-center gap-1 text-sm text-gray-500">
      <button onClick={goHome} className="flex items-center gap-1 hover:text-gray-800">
        <Home className="h-3.5 w-3.5" /> Главная
      </button>
      {stack.map((c, i) => (
        <span key={c.id} className="flex items-center gap-1">
          <ChevronRight className="h-3.5 w-3.5 text-gray-300" />
          <button onClick={() => jumpTo(i)} className={i === stack.length - 1 && !showAll ? 'font-medium text-gray-800' : 'hover:text-gray-800'}>
            {c.name}
          </button>
        </span>
      ))}
      {showAll && (
        <span className="flex items-center gap-1">
          <ChevronRight className="h-3.5 w-3.5 text-gray-300" />
          <span className="font-medium text-gray-800">Все товары</span>
        </span>
      )}
    </div>
  );
      ...
      {list.map((p) => (
        <div key={p.id} className="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white">
          ...
          <div className="flex flex-1 flex-col p-3">
            ...
            <button onClick={() => addToCart(p)} className="mt-3 w-full rounded-lg py-2 text-sm font-semibold transition active:scale-95" style={{ background: accent, color: onAccent }}>
              В корзину
            </button>
          </div>
        </div>
    ...
    <main className="min-h-screen pb-28" style={pageStyle}>
      <div className={`mx-auto px-4 ${isHome ? 'max-w-xl' : 'max-w-3xl'}`}>
        ...
        <div className="relative flex flex-col items-center pt-8 text-center">
          ...
          {store.logo_url ? (
            <img src={store.logo_url} alt={store.name} className={`relative rounded-full bg-white object-cover shadow-sm ${isHome ? 'h-24 w-24' : 'h-16 w-16'}`} />
          ) : (
            <div className={`relative flex items-center justify-center rounded-full font-bold shadow-sm ${isHome ? 'h-24 w-24 text-3xl' : 'h-16 w-16 text-xl'}`} style={{ background: accent, color: onAccent }}>
              {store.name.charAt(0)}
            </div>
          )}
          {isHome && (
        ...
        {isHome ? (
          ...
          <>
            ...
            <div className="mt-3 flex items-center gap-3">
              <button onClick={back} className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">
                <ArrowLeft className="h-4 w-4" />
              </button>
              <h2 className="text-lg font-bold text-gray-900">{showAll ? 'Все товары' : current?.name}</h2>
            </div>
            ...
            {showAll ? (
              ...
              <>
                ...
                {subcats.length === 0 && leafProducts.length === 0 && (
                  <p className="mt-10 text-center text-gray-400">В этом разделе пока пусто.</p>
                )}
              </>
            )}
          </>
      ...
      {/* Плавающая корзина */}
      {totalQty > 0 && (
        <button onClick={() => setCartOpen(true)} className="fixed bottom-5 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full px-6 py-3 font-semibold shadow-xl" style={{ background: accent, color: onAccent }}>
